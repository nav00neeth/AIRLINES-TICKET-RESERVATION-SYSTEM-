		...
		<?php
			if(isset($_POST['Submit']))
			{
				...
				if(empty($data_missing))
				{
					...
					$query1="SELECT total_capacity FROM Jet_Details WHERE jet_id=? and active='Yes'";
					$stmt1=mysqli_prepare($dbc,$query1);
					mysqli_stmt_bind_param($stmt1,"s",$jet_id);
					mysqli_stmt_execute($stmt1);
					mysqli_stmt_bind_result($stmt1,$tot_cnt);
					...
					else if(($seats_bus+$seats_eco)<$tot_cnt)
					{
						header("location: add_flight_details.php?msg=err1");
					}

					else if(($seats_bus+$seats_eco)>$tot_cnt)
					{
						header("location: add_flight_details.php?msg=err2");
					}

					else if(($cnt==1)&&(($seats_bus+$seats_eco)==$tot_cnt))
					{
